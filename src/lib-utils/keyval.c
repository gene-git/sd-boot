...
static int line_to_key_val(ssize_t num_read, char *line, char **key_p, char **val_p) {
    ...
    *key_p = trim_string(*key_p, KV_MAX_KEY_LEN-1);

    if (*val_p) {
        *val_p = trim_string(*val_p, KV_MAX_KEY_LEN-1);
    }
    return 0;
}
...
static int match_key_to_elem(char *key, size_t num_elems, KvElem *elem, size_t *idx_p) {
    ...
    for (size_t idx = 0; idx < num_elems; idx++) {
        if (strncmp(key, elem[idx].key, KV_MAX_KEY_LEN-1) == 0) {
            *idx_p = idx;
            return 0;
        }
    }
