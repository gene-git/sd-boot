 ...
 */
int find_boot_mounts(SdBoot *conf, BootMounts *boot_mounts) {
    int ret = 0;
    struct libmnt_table *mount_table = nullptr;
    struct udev *udev = nullptr;
    ...
    /*
     * We got active not lets mark current as seen by bootctl (if possible).
     */
    ret = boot_mounts_mark_current(conf, boot_mounts);
    if (ret != 0) {
        ret = 0;
        //goto exit;
    }
